...
target_phone_field = 'placeholder="Enter mobile number" style="font-size: 14px; height: 46px; padding: 0 16px; background: rgba(255, 255, 255, 0.02); border: 1.5px solid rgba(255, 255, 255, 0.08); border-radius: 12px; color: #ffffff; width: 100%; box-sizing: border-box; outline: none; transition: all 0.2s ease;" onfocus="this.style.borderColor=\'var(--accent-orange)\'; this.style.background=\'rgba(255, 255, 255, 0.04)\';" onblur="this.style.borderColor=\'rgba(255, 255, 255, 0.08)\'; this.style.background=\'rgba(255, 255, 255, 0.02)\';" />        </div>'

email_field_html = '''placeholder="Enter mobile number" style="font-size: 14px; height: 46px; padding: 0 16px; background: rgba(255, 255, 255, 0.02); border: 1.5px solid rgba(255, 255, 255, 0.08); border-radius: 12px; color: #ffffff; width: 100%; box-sizing: border-box; outline: none; transition: all 0.2s ease;" onfocus="this.style.borderColor='var(--accent-orange)'; this.style.background='rgba(255, 255, 255, 0.04)';" onblur="this.style.borderColor='rgba(255, 255, 255, 0.08)'; this.style.background='rgba(255, 255, 255, 0.02)';" />        </div>
...
if target_phone_field in text:
    text = text.replace(target_phone_field, email_field_html, 1)
    print("Added email field to profile form.")
else:
    print("Warning: target_phone_field not matched!")
...
if "</body>" in text:
    ...
    print("Injected Change Email Modal.")

# 3. Inject JS functions for Change Email and update renderProfileScreen email population
js_functions = '''
...
# Populate prof-edit-email inside renderProfileScreen
...
# 4. Update sendForgotPasswordOtp to use firebase.auth().sendPasswordResetEmail
send_fp_otp_pattern = r"async function sendForgotPasswordOtp\(\)\s*\{[\s\S]*?\n\s*\}\s*\n\s*async function verifyOtpAndResetPassword"
...
text = re.sub(send_fp_otp_pattern, new_send_fp_otp, text, count=1)

with open("app/src/main/assets/index.html", "w", encoding="utf-8") as f:
    f.write(text)

print("Applied profile and authentication module fixes successfully!")
